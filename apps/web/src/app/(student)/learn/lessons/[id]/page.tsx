...
export default function LessonPage() {
  ...
  return (
    <div className="max-w-lg lg:max-w-3xl mx-auto flex flex-col min-h-screen">
      ...
      <div className="sticky top-[3px] bg-white border-b border-gray-100 px-4 lg:px-6 py-3 flex items-center gap-3">
        <button onClick={() => router.back()} className="p-2 -ml-2 rounded-xl text-gray-500 hover:bg-gray-100">
          <ChevronLeft className="w-5 h-5" />
        </button>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="text-xs text-gray-400 uppercase tracking-wide">{t(typeKey)}</p>
            <span className="flex items-center gap-1 text-xs text-gray-400">
              <Clock className="w-3 h-3" />~{readMin} {lang === 'ru' ? 'мин' : 'min'}
            </span>
          </div>
          <h1 className="font-bold truncate">{translateTitle(lesson.title)}</h1>
        </div>
      </div>
