  ...
  return (
    <div className="mt-6 space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs font-medium text-gray-400 uppercase">{lang === 'ru' ? 'Торги' : 'Negotiation'}</p>
        <p className="text-xs text-gray-400">{roundIdx + 1}/{ROUNDS.length}</p>
      </div>

      {/* Load info */}
      <div className="card p-3 bg-gray-900 text-white border-0">
        <p className="text-xs text-gray-400">{r.personality} · {r.name}</p>
        <p className="font-semibold">{r.load}</p>
        <p className="text-xs text-gray-400">{r.miles} mi · Market: ${r.market.toLocaleString()}</p>
      </div>

      {/* Broker offer */}
      <div className="bg-gray-100 rounded-xl p-3">
        <p className="text-xs text-gray-500 mb-1">{lang === 'ru' ? 'Предложение брокера' : 'Broker offers'}:</p>
        <p className="text-xl font-bold text-gray-900">${brokerOffer.toLocaleString()}</p>
        <p className="text-xs text-gray-400">${(brokerOffer / r.miles).toFixed(2)}/mi</p>
      </div>
      ...
      {phase === 'offer' && (
        <>
          {/* Slider */}
          <div>
            <p className="text-xs text-gray-500 mb-2">{lang === 'ru' ? 'Ваше предложение' : 'Your counter-offer'}:</p>
            ...
            <p className="text-xs text-gray-400 text-center">${(sliderVal / r.miles).toFixed(2)}/mi</p>
          </div>

          {/* P&L */}
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="card p-2">
              <p className="text-xs text-gray-400">{lang === 'ru' ? 'Расходы' : 'Cost'}</p>
              <p className="font-bold text-sm">${carrierCost.toLocaleString()}</p>
            </div>
            <div className="card p-2">
              <p className="text-xs text-gray-400">{lang === 'ru' ? 'Маржа' : 'Margin'}</p>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={acceptBrokerOffer} className="btn-ghost border border-gray-200 flex-1">
      ...
      {phase === 'thinking' && (
        <div className="text-center py-6">
          ...
          <p className="text-sm text-gray-500">{r.name} {lang === 'ru' ? 'думает...' : 'is thinking...'}</p>
        </div>
      )}
        ...
        <div className="card p-4 text-center space-y-2">
          {dealRate > 0 ? (
            <>
              ...
              <p className="text-sm text-gray-500">${(dealRate / r.miles).toFixed(2)}/mi · {Math.round(((dealRate - carrierCost) / dealRate) * 100)}% margin</p>
            </>
          ) : (
