...
export default function ExamsPage() {
  ...
  if (isLoading) return <div className="px-4 pt-14 lg:pt-6 pb-24 max-w-lg mx-auto animate-pulse space-y-3">{[1,2,3].map(i => <div key={i} className="h-16 bg-gray-100 rounded-2xl" />)}</div>;
    ...
    return (
      <div className="px-4 pt-14 lg:pt-6 pb-24 max-w-lg mx-auto text-center">
        <div className="card p-8">
          ...
          <h2 className="text-lg font-bold text-gray-900">{lang === 'ru' ? 'Экзамены закрыты' : 'Exams Locked'}</h2>
  ...
  return (
    <div className="px-4 pt-14 lg:pt-6 pb-24 max-w-lg lg:max-w-2xl mx-auto">
      <h1 className="text-xl font-bold text-gray-900 mb-1">{lang === 'ru' ? 'Ежедневные экзамены' : 'Daily Exams'}</h1>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                available && !completed && 'bg-white border-gray-200 hover:border-emerald-300 hover:shadow-sm cursor-pointer',
                locked && 'bg-gray-50 border-gray-100 opacity-50 cursor-default',
              )}
            >
              <div className={cn(
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-gray-900">{lang === 'ru' ? 'Экзамен' : 'Exam'} #{num}</p>
              ...
              {available && !completed && <ChevronRight className="w-4 h-4 text-gray-300" />}
...
function ExamSession({ examNumber, onBack }: { examNumber: number; onBack: () => void }) {
  ...
  if (isLoading) return <div className="p-8 text-center animate-pulse"><div className="h-8 bg-gray-100 rounded-xl w-48 mx-auto" /></div>;
    ...
    return (
      <div className="px-4 pt-14 lg:pt-6 pb-24 max-w-lg mx-auto text-center">
        <div className="card p-8">
          ...
          <p className="text-3xl font-bold font-mono text-gray-900">{result.score}%</p>
  ...
  return (
    <div className="px-4 pt-14 lg:pt-6 pb-24 max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-4">
        <button onClick={onBack} className="text-sm text-gray-500 cursor-pointer">{lang === 'ru' ? '← Назад' : '← Back'}</button>
        <span className="text-xs font-mono text-gray-400">{current + 1}/{questions.length}</span>
      </div>
      ...
      <p className="text-gray-900 font-semibold mb-4">{q.text}</p>
      <div className="space-y-2 mb-6">
        {q.options.map(opt => {
          ...
          return (
            ...
            }} className={cn('w-full text-left px-4 py-3 rounded-xl border-2 transition-all text-sm cursor-pointer', isSelected ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300')}>
              {opt.text}
            </button>
          );
        })}
      ...
      <div className="flex gap-2">
        {current > 0 && <button onClick={() => setCurrent(c => c - 1)} className="btn-ghost border border-gray-200 flex-1 cursor-pointer">{lang === 'ru' ? 'Назад' : 'Back'}</button>}
        {isLast ? (
