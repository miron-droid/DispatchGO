  ...
  return (
    <div className="mt-6 space-y-3">
      <div className="card p-3 bg-gradient-to-r from-brand-50 to-white">
        <p className="text-sm font-bold text-gray-900">🚛 {lang === 'ru' ? 'Ваш грузовик' : 'Your Truck'}: {TRUCK.equipment}</p>
        <p className="text-xs text-gray-500">📍 {TRUCK.city}</p>
        <p className="text-xs text-gray-400 mt-1">{lang === 'ru' ? 'Выберите 3 лучших груза' : 'Pick the 3 best loads'}</p>
      </div>

      {/* Filter */}
      <div className="flex items-center gap-3 text-xs">
        <Filter className="w-4 h-4 text-gray-400" />
        <span className="text-gray-500">Max DH:</span>
        ...
        <span className="font-mono text-gray-700 w-12">{maxDH} mi</span>
      </div>
              ...
              <tr key={l.id} className={cn(
                ...
                <td className="p-2">
                  <button onClick={() => toggleSelect(l.id)}>
                    <Star className={cn('w-4 h-4', selected.has(l.id) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300')} />
                  </button>
                </td>
                <td className="p-2">
                  <p className="font-medium text-gray-900">{l.origin}</p>
                  <p className="text-gray-500">→ {l.dest}</p>
                </td>
