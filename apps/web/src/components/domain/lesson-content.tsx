...
function useScrollReveal(containerRef: React.RefObject<HTMLDivElement | null>) {
  useEffect(() => {
    ...
    const targets = el.querySelectorAll<HTMLElement>(
      'h2, h3, p, blockquote, figure, ul, ol, .scroll-reveal'
    );
...
export function LessonContent({ content }: { content: Content }) {
  const { t } = useLang();
  if (!content) return <p className="text-gray-400 italic">{t('lesson_no_content')}</p>;
...
function TextRenderer({ c }: { c: TextContent }) {
  ...
  return (
    <div>
      <div
        ref={ref}
        className="lesson-body text-gray-800 leading-relaxed lg:text-[17px] lg:leading-8
          [&_h2]:text-xl lg:[&_h2]:text-2xl [&_h2]:font-bold [&_h2]:text-gray-900 [&_h2]:mb-4 [&_h2]:mt-6 lg:[&_h2]:mt-8 first:[&_h2]:mt-0
          [&_h3]:text-lg lg:[&_h3]:text-xl [&_h3]:font-semibold [&_h3]:text-gray-900 [&_h3]:mb-3 [&_h3]:mt-5 lg:[&_h3]:mt-7
          [&_p]:mb-4 [&_p]:text-gray-700 [&_p]:leading-7 lg:[&_p]:leading-8
          [&_blockquote]:border-l-4 [&_blockquote]:border-brand-500 [&_blockquote]:pl-4 lg:[&_blockquote]:pl-6 [&_blockquote]:py-1 [&_blockquote]:my-4 lg:[&_blockquote]:my-6 [&_blockquote]:text-gray-600 [&_blockquote]:italic
          ...
          [&_li]:text-gray-700
          [&_figure]:my-6 lg:[&_figure]:my-8 [&_figure]:overflow-hidden [&_figure]:rounded-xl lg:[&_figure]:rounded-2xl [&_figure]:border [&_figure]:border-gray-100
          [&_img]:w-full [&_img]:h-auto [&_img]:block [&_img]:object-cover lg:[&_img]:max-h-[500px] lg:[&_img]:object-contain lg:[&_img]:bg-gray-50
          [&_figcaption]:text-xs [&_figcaption]:text-gray-500 [&_figcaption]:italic [&_figcaption]:text-center [&_figcaption]:px-4 [&_figcaption]:py-2 [&_figcaption]:bg-gray-50"
...
function QuizRenderer({ questions }: { questions: QuizQuestion[] }) {
  ...
  if (finished) {
    return (
      <div className="mt-8 rounded-2xl border border-gray-200 bg-white p-6 lg:p-8 text-center space-y-4 lg:max-w-xl lg:mx-auto">
        <p className="text-4xl font-bold text-gray-900">{percent}%</p>
        <p className="text-sm text-gray-500">{score} {t('quiz_out_of')} {total} {t('quiz_correct')}</p>

        {passed ? (
          <>
            <p className="text-green-600 font-semibold text-lg">{t('quiz_goal_achieved')}</p>
            <p className="text-sm text-gray-500">{t('quiz_proceed')}</p>
          </>
        ) : (
          <>
            <p className="text-red-500 font-semibold text-lg">{t('quiz_goal_not_achieved')}</p>
            {attempt + 1 >= MAX_ATTEMPTS ? (
              <p className="text-sm text-gray-500">{t('quiz_review_material')}</p>
            ) : (
              <button
                onClick={retry}
  ...
  return (
    <div className="mt-8 space-y-4 lg:max-w-xl lg:mx-auto">
      <div className="flex items-center justify-between">
        <p className="text-xs font-medium text-gray-400 uppercase tracking-wide">{t('quiz_title')}</p>
      ...
      <p className="text-gray-900 font-semibold leading-snug pt-1">{q.text}</p>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                showResult && !isSelected && !isCorrect && 'border-gray-100 opacity-40',
              )}
            >
              <span className={cn(
                ...
                showResult && !isSelected && !isCorrect && 'border-gray-200 text-gray-300',
              )}>
                {OPTION_LABELS[i]}
              </span>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
        <p className="text-sm font-medium text-amber-800 mb-1">{t('case_scenario')}</p>
        <p className="text-gray-800 leading-relaxed">{c.scenario}</p>
      </div>

      <p className="font-semibold text-gray-700">{t('case_what_would_you_do')}</p>
      <div className="space-y-2">
        {c.options.map((opt, i) => {
