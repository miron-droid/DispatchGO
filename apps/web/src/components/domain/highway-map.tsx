  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 via-[#0f172a] to-gray-900 p-4 lg:p-5">
      ...
      <div className="relative flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2.5">
          <div className="text-right">
            <p className="text-lg font-bold text-white">{progressPct}<span className="text-xs text-gray-500 font-normal">%</span></p>
          </div>
          <div className="relative w-9 h-9">
            ...
            <span className="absolute inset-0 flex items-center justify-center text-[9px] font-bold text-gray-400">{completedCount}/{total}</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative flex items-center justify-between mt-3 pt-3 border-t border-gray-800/60">
        ...
        <p className="text-[9px] text-gray-600 font-mono">{completedCount * 4} / 36 {lang === 'ru' ? 'уроков' : 'lessons'}</p>
      </div>
    </div>
  );
}
