        ...
        {Array.from({ length: total }, (_, i) => (
          <div
            ...
            className={cn(
              ...
              i < step ? 'bg-brand-500 scale-110' : 'bg-gray-200',
            )}
          />
        ))}
      </div>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                !showResult && 'border-gray-200 hover:border-brand-300 active:border-brand-500',
                showResult && isSelected && isCorrect && 'border-green-500 bg-green-50',
                showResult && isSelected && !isCorrect && 'border-red-400 bg-red-50',
                showResult && !isSelected && 'border-gray-100 opacity-40',
              )}
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-bold text-gray-900">{load.route}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-sm text-gray-700 font-medium">{load.rate}</span>
                    <span className="text-gray-300">•</span>
                    <span className="text-sm text-gray-400">{load.miles}</span>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-400 mt-1">RPM</p>
                </div>
              </div>
            </button>
          );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Broker</p>
      ...
      <div className="bg-green-50 rounded-2xl p-5 border border-green-100 space-y-3">
        ...
        {details.map((d, i) => (
          <div key={i} className="flex justify-between items-start gap-2 text-sm">
            ...
            <span className="text-gray-800 font-semibold text-right">{d.value}</span>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
        <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Check Call ✓</p>
      ...
      <div className="bg-green-50 rounded-2xl border border-green-100 p-4 space-y-2">
        ...
        {financials.map((f, i) => (
          <div key={i} className={cn('flex justify-between text-sm', f.bold && 'border-t border-green-200 pt-2 mt-1')}>
            ...
            <span className={f.bold ? 'font-bold text-green-700' : 'font-semibold text-gray-700'}>{f.value}</span>
          </div>
        ))}
      </div>
