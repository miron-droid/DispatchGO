...
const ZONE_BADGE: Record<ZoneType, { ru: string; en: string; dot: string; cls: string }> = {
  ...
  UNKNOWN: { ru:'Нет данных',           en:'No Data',          dot:'#94a3b8', cls:'bg-gray-100 text-gray-500'                           },
};
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1">
          {(['PT','MT','CT','ET'] as TZ[]).map(tz => (
            <div key={tz} className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-gray-400">{TZ_META[tz].offset}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <p className="font-bold text-gray-900 text-base leading-tight">
            {STATE_NAMES[state]}
            <span className="font-normal text-gray-400 text-sm ml-1.5">({state})</span>
          </p>
          <p className="text-xs text-gray-400 mb-2.5">{ru ? zone.titleRu : zone.titleEn}</p>
          ...
          <p className="text-sm text-gray-600 leading-relaxed">{ru ? zone.descRu : zone.descEn}</p>
          <div className="mt-3 inline-flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-1.5">
            <span className="text-xs text-gray-400">{ru ? 'Средняя ставка:' : 'Avg rate:'}</span>
            <span className="text-sm font-bold text-gray-900">{zone.avgRate}</span>
          </div>
        </div>
        <button onClick={onClose}
          className="shrink-0 w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 hover:bg-gray-200 transition-colors">
    ...
    <>
      ...
      <div className="mt-6 rounded-2xl overflow-hidden border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          <div>
            <p className="font-bold text-gray-900 text-sm">{ru ? 'Карта зон США' : 'US Freight Zones Map'}</p>
            <p className="text-xs text-gray-400 mt-0.5">{ru ? 'Нажмите на штат для деталей' : 'Click a state for details'}</p>
          </div>
        ...
        <div className="relative bg-slate-50" style={{ paddingTop: '62%' }}>
          ...
          {selected && (
            <div className="absolute bottom-2 right-2 z-10 w-72 rounded-xl shadow-xl overflow-hidden border border-gray-200 bg-white">
          ...
          <Dialog.Overlay className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-3">
            ...
            >
              ...
              <div className="flex-1 bg-slate-50 relative min-h-0" style={{ minHeight: '300px' }}>
                ...
                {expandedSel && (
                  <div className="absolute bottom-4 right-4 z-10 w-80 rounded-xl shadow-xl overflow-hidden border border-gray-200 bg-white">
