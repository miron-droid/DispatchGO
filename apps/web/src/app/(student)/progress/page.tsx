    ...
    <div className="max-w-lg lg:max-w-3xl mx-auto px-4 lg:px-6 pt-6 pb-8">
      ...
      <div className="card p-5 mb-4 bg-gradient-to-br from-gray-900 to-gray-800 text-white border-0">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-400 text-sm">{g.totalXP.toLocaleString()} XP</p>
          </div>
          {g.streak > 0 && (
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="card p-3 text-center">
          <p className="text-2xl font-bold text-gray-900">{g.totalLessons}</p>
          <p className="text-xs text-gray-400">{lang === 'ru' ? 'уроков' : 'lessons'}</p>
        </div>
        <div className="card p-3 text-center">
          <p className="text-2xl font-bold text-gray-900">{g.streak}</p>
          <p className="text-xs text-gray-400">{lang === 'ru' ? 'дней подряд' : 'day streak'}</p>
        </div>
        <div className="card p-3 text-center">
          <p className="text-2xl font-bold text-gray-900">{g.unlockedAchievements.length}</p>
          <p className="text-xs text-gray-400">{lang === 'ru' ? 'достижений' : 'badges'}</p>
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">{l.minXP.toLocaleString()} XP</p>
                </div>
