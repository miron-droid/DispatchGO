...
function ManagerShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-surface-secondary">
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 w-56 bg-white border-r border-gray-100 z-50 flex-col">
        <div className="px-4 py-4 border-b border-gray-100">
          <Logo size={32} textSize="md" />
          <p className="text-[10px] text-gray-400 mt-1 ml-[42px]">Manager Panel</p>
        </div>
          ...
          {NAV.map(item => (
            <Link
              ...
              className={cn(
                ...
                isActive(item) ? 'bg-emerald-50 text-emerald-600' : 'text-gray-500 hover:bg-gray-50',
              )}
            >
      ...
      {/* Mobile header */}
      <header className="lg:hidden fixed top-0 left-0 right-0 bg-white/90 backdrop-blur-md border-b border-gray-100 z-50 h-12 flex items-center px-4 justify-between">
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 top-12 bg-white z-40 p-4 space-y-2">
          {NAV.map(item => (
            <Link
              key={item.href}
              href={item.href}
              ...
              className={cn(
                ...
                isActive(item) ? 'bg-emerald-50 text-emerald-600' : 'text-gray-600',
              )}
            >
