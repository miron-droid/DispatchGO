...
function getContent(chapter: number, lesson: number): object {
  const contents: Record<string, object> = {
    '1-1': {
      ...
      bodyRu: `
        ...
        <p><strong>Shipper</strong> (грузоотправитель) — компания с товаром для доставки. <strong>Broker</strong> (брокер) — посредник, публикующий груз на <strong>load boards</strong> (биржах грузов: DAT, Truckstop). <strong>Carrier</strong> (перевозчик) — владелец грузовиков, нанимающий водителей.</p>
      ...
      body: `
        ...
        <p>The <strong>shipper</strong> has goods that need to move. The <strong>broker</strong> is an intermediary who finds a carrier for the load — posting it on <strong>load boards</strong> (DAT, Truckstop). The <strong>carrier</strong> owns trucks and employs drivers who perform the actual transport.</p>
    ...
    '1-2': {
      ...
      body: `
        ...
        <p>The broker connects shippers with carriers — owning no trucks. Posts loads on <strong>DAT</strong> and <strong>Truckstop (ITS)</strong>, negotiates rates, issues the <strong>rate confirmation (rate con)</strong> — a binding written agreement both parties sign before the truck moves.</p>
        <blockquote>
        ...
        <ul>
          ...
          <li><strong>Negotiation:</strong> Always counter the first price; know the DAT market rate before calling</li>
      ...
      bodyRu: `
        ...
        <p>Брокер соединяет shippers с carriers, не владея транспортом. Публикует грузы на <strong>DAT</strong> и <strong>Truckstop (ITS)</strong>, оформляет <strong>rate con</strong> — обязывающий документ, который обе стороны подписывают до начала перевозки.</p>
        <blockquote>
        ...
        <ul>
          ...
          <li><strong>Переговоры:</strong> Всегда делать контрпредложение; знать ставку DAT до звонка</li>
    ...
    '1-3': {
      ...
      body: `
        ...
        <p>Search DAT/Truckstop filtered by driver's pickup location. Evaluate by lane direction (strong outbound market at destination?) and always calculate RPM — not just total dollars.</p>
        <blockquote>
        ...
        <blockquote>
          ...
          Result: +$225 above posted rate. Check DAT rate history before calling. Never accept the first price.
        </blockquote>
      ...
      bodyRu: `
        ...
        <p>Ищем на DAT/Truckstop с фильтром по позиции водителя. Оцениваем lane (сильный ли рынок в точке доставки?) и всегда считаем RPM — не только итоговую сумму.</p>
        <blockquote>
        ...
        <blockquote>
          ...
          Результат: +$225 сверх выставленной ставки. Проверьте историю DAT до звонка. Никогда не принимайте первую цену.
        </blockquote>
    ...
    '3-3': {
      ...
      body: `
        ...
        <p>DAT load: 12 pallets | 26ft SB/LG/RESI | $1,650 → Marcus (only match). Broker: LG accessorial not included. Alex: "Need $1,800 — residential, liftgate both ends." Broker: "$1,750." Done.</p>
      ...
      bodyRu: `
        ...
        <p>DAT: 12 поддонов | 26ft SB/LG/RESI | $1 650 → Маркус (единственное совпадение). Брокер: accessorial за LG не включён. Алекс: «Нужно $1 800 — жилая, лифтгейт с обеих сторон.» Брокер: «$1 750.» Договорились.</p>
  ...
  // ── Chapter 5: CARGO ETL Application ─────────────────────────────────────
  contents['5-1'] = {
    type: 'text',
...
<blockquote>The app works in sync with your Gmail account. Broker communication happens in email — CARGO ETL reads those emails and surfaces the relevant load data automatically.</blockquote>`,
...
<blockquote>Приложение работает в связке с вашим Gmail. Общение с брокерами происходит в почте — CARGO ETL читает эти письма и автоматически отображает данные по грузам.</blockquote>`,
    quiz: {
      questions: [
...
<figure><img src="/cargo/fig8.png" alt="Gmail notification" /><figcaption>Fig. 8 — Green notification icon (bottom-right): broker replied to your email</figcaption></figure>`,
...
<figure><img src="/cargo/fig8.png" alt="Уведомление Gmail" /><figcaption>Рис. 8 — Зелёная иконка уведомления (правый нижний угол): брокер ответил на ваше письмо</figcaption></figure>`,
    quiz: {
      questions: [
...
<blockquote>The broker who trusts you sends you loads without shopping around. Building that trust is the long-term goal of every broker relationship.</blockquote>`,
...
<blockquote>Брокер, который вам доверяет, отдаёт груз без поиска альтернатив. Выстроить это доверие — долгосрочная цель каждых брокерских отношений.</blockquote>`,
    quiz: {
      questions: [
...
<blockquote>Without POD — no payment. This is the most critical document at the end of every load.</blockquote>`,
...
<blockquote>Без POD — нет оплаты. Это самый критический документ в конце каждого груза.</blockquote>`,
    quiz: {
      questions: [
...
contents['8-3'] = {
      type: 'text',
      bodyRu: `
        <h2>Симуляция торгов — практика в реальных условиях</h2>
        <p>В этой симуляции вы пройдёте полный цикл бронирования груза: от анализа груза на бирже до подтверждения Rate Confirmation. Принимайте решения как настоящий диспетчер.</p>
        <blockquote>
          <strong>Сценарий:</strong> На бирже появился груз — Мосини, WI → Колумбия-Сити, IN, 412 миль. Водитель в 14 милях от места загрузки. Пройдите все этапы торгов.
        </blockquote>
      `,
      body: `
        <h2>Bidding Simulation — Practice in Real Conditions</h2>
        <p>In this simulation you will go through the full load booking cycle: from analyzing a load on the board to confirming the Rate Confirmation. Make decisions like a real dispatcher.</p>
        <blockquote>
          <strong>Scenario:</strong> A load appeared on the board — Mosinee, WI → Columbia City, IN, 412 miles. Driver is 14 miles from pickup. Go through all bidding stages.
        </blockquote>
      `,
