  ...
  if (finished) {
    return (
      <div className="mt-6 card p-6 text-center space-y-3">
        ...
        <p className="text-sm text-gray-500">{lang === 'ru' ? 'правильных с первой попытки' : 'correct on first try'}</p>
  ...
  return (
    <div className="mt-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <p className="text-xs text-gray-400">{round + 1}/{ROUNDS.length}</p>
      </div>
        ...
        {EQUIPMENT.map(eq => {
          ...
          return (
            ...
            >
              ...
              <p className="text-[10px] text-gray-400 mt-0.5">{eq.specs}</p>
            </button>
          );
        })}
      </div>
