...
const ROUNDS: Round[] = [
  {
    ...
    options: [
      ...
      { id: 'b', text: "DAT shows $2.35/mile average on this lane. I'd need at least $2,150 to make it work.", score: 10, reaction: "Fair point. I can stretch to $1,950. That's about all budget allows." },
  ...
  if (phase === 'done') {
    return (
      <div className="mt-6 card p-6 text-center space-y-3 bg-gray-900 text-white border-0">
        ...
        <p className="text-sm text-gray-400">{lang === 'ru' ? 'Звонок завершён' : 'Call ended'} · {formatTime(callTime)}</p>
        <p className="text-3xl font-bold">{pct}%</p>
        <p className="text-sm">{pct >= 80 ? '🌟 Pro negotiator!' : pct >= 50 ? '👍 Solid call' : '📚 Practice more'}</p>
        <p className="text-xs text-gray-500">{lang === 'ru' ? 'Итоговая ставка' : 'Final rate'}: $2,100 | RPM: $2.28</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-6 rounded-2xl overflow-hidden bg-gray-900 text-white">
      ...
      <div className="px-4 pt-5 pb-3 text-center">
        ...
        <p className="text-xs text-gray-500 mt-1">Chicago → Dallas · 920 mi · Dry Van</p>
      </div>
      ...
      {phase === 'reaction' && selectedOpt && (
        <div className="px-4 pb-3 space-y-2">
          ...
          <div className={cn('text-xs text-center py-1', selectedOpt.score > 0 ? 'text-green-400' : selectedOpt.score < 0 ? 'text-red-400' : 'text-gray-500')}>
      ...
      {phase === 'tip' && current?.tip && (
        <div className="px-4 pb-3">
          ...
          <button onClick={nextRound} className="w-full mt-2 py-2 text-xs text-gray-400 hover:text-white">
